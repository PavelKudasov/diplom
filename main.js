...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function app(canvasEl) {
    ...
    const VERTEX_PICK_THRESHOLD = 18;
    const EDGE_PICK_THRESHOLD = 20; // пикселей по экрану для уверенного выбора ребра
    ...
    function buildMedian(shape, anchorIndex) {
      ...
      if (!mid) return null;
      return { type: 'median', from: shape.worldToLocal(vertex), to: shape.worldToLocal(mid) };
    }
    ...
    function buildBisector(shape, anchorIndex) {
      ...
      if (!isFinite(v1.x) || !isFinite(v1.y) || !isFinite(v2.x) || !isFinite(v2.y)) return null;
      ...
      return { type: 'bisector', from: shape.worldToLocal(B), to: shape.worldToLocal(endPoint) };
    }
    ...
    let rayBound = getCanvasDiag() * 2; // ограничитель для построений на бесконечных линиях/лучах
    ...
    function findVertexOrEdgePoint(clientX, clientY, shape) {
      ...
      if (typeof shape.getVerticesWorld === 'function') {
        ...
        if (bestV.d <= VERTEX_PICK_THRESHOLD) {
        ...
        if (bestEdge.proj && bestEdge.d <= EDGE_PICK_THRESHOLD) {
    ...
    function render() {
      ...
      if (constructionMode && constructionTemp.points.length > 0) {
        ...
        constructionTemp.points.forEach((p, idx) => {
          ...
          ctx.fillStyle = '#000';
          ctx.font = '12px Arial';
          ctx.fillText((idx + 1).toString(), s.x + 10, s.y - 10);
          ctx.fillStyle = '#ff0000';
        });
        ctx.restore();
      }
    ...
    canvasEl.addEventListener('pointerdown', (e) => {
      ...
      if (activeShape && activeShape._handles) {
        ...
        for (const h of activeShape._handles) {
          if (Math.hypot(sc.x - h.x, sc.y - h.y) < 12) { activeHandle = h; return; }
        }
      }
      if (constructionMode) {
        if (!activeShape) { console.info('Сначала выберите фигуру для построения.'); return; }
        ...
        // Если для медианы/биссектрисы не выбрана вершина — привязываем к ближайшей вершине
        if ((constructionType === 'median' || constructionType === 'bisector') && constructionTemp.points.length > 0 && typeof activeShape.getVerticesWorld === 'function') {
        ...
        // Разрешаем построения только для фигур с вершинами
        const okForBuild = figuresWithVertices.some(cls => activeShape instanceof cls) || typeof activeShape.getVerticesWorld === 'function';
        if (!okForBuild) {
        ...
        // Для угла подменяем виртуальные вершины: вершина (0) и удлинённые точки лучей (1,2)
        if (activeShape instanceof AngleShape) {
          ...
          // если выбрано меньше 3 точек, подсказка
          if (constructionTemp.points.length === 1) console.info('Выберите вторую точку на луче для построения.');
        ...
        if (constructionType === 'median') {
          const last = constructionTemp.points[constructionTemp.points.length - 1];
          if (last.vertexIndex != null) {
            const cons = buildMedian(activeShape, last.vertexIndex);
            if (cons) {
              cons.id = uid('cons');
          ...
          } else { console.info('Медиана строится из вершины — кликните ближе к вершине фигуры.'); }
          return;
        }
        if (constructionType === 'bisector') {
          const last = constructionTemp.points[constructionTemp.points.length - 1];
          if (last.vertexIndex != null) {
            const cons = buildBisector(activeShape, last.vertexIndex);
            if (cons) {
              cons.id = uid('cons');
              activeShape.constructions.push(cons);
              constructionTemp.points = [];
          ...
          } else { console.info('Биссектриса строится из вершины — кликните ближе к вершине фигуры.'); }
          return;
        }
        if (constructionType === 'perpBisector') {
          ...
          } else { console.info('Выберите вторую точку для перпендикулярного биссектора.'); }
          return;
        }
        if (constructionType === 'line') {
          if (constructionTemp.points.length >= 2) {
            ...
            if (cons) {
              ...
              console.info('Не удалось построить прямую по двум точкам. Попробуйте выбрать вершины или точки ближе к фигуре.');
              constructionTemp.points = [];
              constructionMode = false;
              constructionType = null;
            }
          } else { console.info('Выберите вторую точку для прямой.'); }
          return;
        }
        if (constructionType === 'parallel') {
          if (constructionTemp.points.length === 1) {
            const first = constructionTemp.points[0];
            if (first.edge) { console.info('Теперь кликните точку, через которую провести параллельную прямую.'); return; }
            else { console.info('Для параллельной прямой сначала кликните по ребру (или рядом с ним).'); constructionTemp.points = []; return; }
            ...
            if (first.edge) {
              ...
              if (cons) {
                ...
                console.info('Не удалось построить параллельную: нет пересечения с границей фигуры.');
                constructionTemp.points = [];
                constructionMode = false;
                constructionType = null;
              }
            } else { console.info('Первый клик должен быть по ребру (или рядом с ним).'); constructionTemp.points = []; }
          }
          return;
        }
        if (constructionType === 'section') {
        ...
        if (!hitted) {
          try {
            if (typeof s.getBoundingBox === 'function') {
              ...
              // Увеличиваем padding для фигур с построениями
              const pad = s.constructions && s.constructions.length > 0 ? 20 : 12;
              if (bb && !Number.isNaN(bb.x)) {
                if (w.x >= bb.x - pad && w.x <= bb.x + bb.w + pad && w.y >= bb.y - pad && w.y <= bb.y + bb.h + pad) { hitted = true; }
              }
            }
          } catch (err) {}
        }
    ...
    canvasEl.addEventListener('pointerleave', endPointerInteraction);
    ...
    (function propsModule() {
      ...
      propsPanel.innerHTML = `
        <div style="font-weight:600;margin-bottom:6px">Свойства фигуры</div>
        <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
          <button id="btn-scale-dec" type="button" style="width:32px;height:32px">➖</button>
          <button id="btn-scale-inc" type="button" style="width:32px;height:32px">➕</button>
          <button id="btn-rot-dec" type="button" style="width:32px;height:32px">⟲</button>
          <button id="btn-rot-inc" type="button" style="width:32px;height:32px">⟳</button>
          <button id="prop-reset" type="button" style="min-width:130px">↺ Сброс</button>
        </div>
        <div id="prop-value" style="margin-top:8px;color:#333;font-size:13px">Scale: 100%; Rot: 0°</div>
        <div id="prop-info" style="margin-top:8px;color:#444;font-size:13px"></div>
        <div id="shape-specific" style="margin-top:8px"></div>
      `;
      ...
      const specific = propsPanel.querySelector('#shape-specific');
      const propInfo = propsPanel.querySelector('#prop-info');
      const btnReset = propsPanel.querySelector('#prop-reset');
      const propValueLabel = propsPanel.querySelector('#prop-value');

      function updatePropsPanel(shape) {
        if (!shape) {
          specific.innerHTML = '<div style="color:#666">Фигура не выбрана</div>';
          propValueLabel.textContent = 'Scale: 100%; Rot: 0°';
          propInfo.innerHTML = '';
          return;
        }
        const scaleText = Math.round((shape.scale || 1) * 100);
        const rotText = Math.round((shape.rotation || 0) * 180 / Math.PI) % 360;
        propValueLabel.textContent = `Scale: ${scaleText}% ; Rot: ${rotText}°`;
        const details = [];
        details.push(`<div>Тип: ${shape.constructor.name.replace('Shape', '')}</div>`);
        details.push(`<div>Координаты: x=${shape.x.toFixed(1)}, y=${shape.y.toFixed(1)}</div>`);
        details.push(`<div>Scale: ${scaleText}%</div>`);
        details.push(`<div>Rotation: ${rotText}°</div>`);
        if (shape instanceof LineShape || shape instanceof RayShape) {
          const globalAngle = (shape.angle + (shape.rotation || 0)) * 180 / Math.PI;
          details.push(`<div>Угол линии: ${((globalAngle % 360) + 360) % 360 .toFixed(1)}°</div>`);
          details.push(`<div>Длина (в ячейках): ${shape.cells}</div>`);
        }
        if (shape instanceof SegmentShape) {
          details.push(`<div>Длина сегмента: ${shape.length.toFixed(1)}</div>`);
          const segAngle = (shape.rotation || 0) * 180 / Math.PI;
          details.push(`<div>Угол сегмента: ${((segAngle % 360) + 360) % 360 .toFixed(1)}°</div>`);
        }
        if (shape instanceof AngleShape) {
          details.push(`<div>Угол: ${(shape.angle * 180 / Math.PI).toFixed(1)}°</div>`);
          details.push(`<div>Базовый угол: ${(shape.base * 180 / Math.PI).toFixed(1)}°</div>`);
        }
        if (shape instanceof CircleShape) { details.push(`<div>Радиус: ${shape.r.toFixed(1)}</div>`); }
        if (shape instanceof RectShape) {
          details.push(`<div>Ширина: ${shape.w.toFixed(1)}</div>`);
          details.push(`<div>Высота: ${shape.h.toFixed(1)}</div>`);
        }
        if (shape instanceof SquareShape) { details.push(`<div>Сторона: ${shape.w.toFixed(1)}</div>`); }
        if (shape instanceof PolygonShape) { details.push(`<div>Стороны: ${shape.sides}</div>`); }
        if (shape.constructions && shape.constructions.length > 0) {
          details.push(`<div style="margin-top:6px;font-weight:600">Построения: ${shape.constructions.length}</div>`);
          shape.constructions.forEach((c, idx) => {
            ...
            if (c.from && c.to) {
              ...
              extra += ` · длина=${len.toFixed(1)} · угол=${((angleDeg % 360) + 360) % 360 .toFixed(1)}°`;
            }
            if (c.type === 'section' && c.points && c.points.length >= 3) {
              // площадь треугольника из первых трёх точек (локальные коорд), как приближение
              const [p1, p2, p3] = c.points;
              const area = Math.abs(
                (p2.x - p1.x) * (p3.y - p1.y) -
                (p3.x - p1.x) * (p2.y - p1.y)
              ) / 2;
              extra += ` · площадь≈${area.toFixed(1)}`;
            }
            details.push(`<div style="font-size:12px;color:#555">${idx + 1}. ${type}${extra}</div>`);
          });
        }
        propInfo.innerHTML = details.join('');
        specific.innerHTML = '';
        if (shape instanceof RectShape) {
          specific.innerHTML = `
            <div><label>Width: <input id="prop-w" type="number" value="${shape.w}" style="width:80px"></label></div>
            <div><label>Height: <input id="prop-h" type="number" value="${shape.h}" style="width:80px"></label></div>
          `;
          specific.querySelector('#prop-w').addEventListener('change', (e) => { shape.w = Number(e.target.value) || shape.w; });
          specific.querySelector('#prop-h').addEventListener('change', (e) => { shape.h = Number(e.target.value) || shape.h; });
        } else if (shape instanceof CircleShape) {
          specific.innerHTML = `<div><label>Radius: <input id="prop-r" type="number" value="${shape.r}" style="width:80px"></label></div>`;
          specific.querySelector('#prop-r').addEventListener('change', (e) => { shape.r = Number(e.target.value) || shape.r; });
        } else if (shape instanceof PolygonShape) {
          specific.innerHTML = `<div><label>Sides: <input id="prop-sides" type="number" min="3" value="${shape.sides}" style="width:80px"></label></div>`;
          specific.querySelector('#prop-sides').addEventListener('change', (e) => {
            const n = Number(e.target.value) || shape.sides;
            shape.sides = Math.max(3, n | 0);
            shape.vertexNames = Array.from({ length: shape.sides }, (_, i) => String.fromCharCode(65 + i));
          });
        } else if (shape instanceof AngleShape) {
          specific.innerHTML = `
            <div><label>Величина (°): <input id="prop-angle" type="number" min="1" max="359" value="${(shape.angle * 180/Math.PI).toFixed(1)}" style="width:80px"></label></div>
            <div><label>База (°): <input id="prop-base" type="number" min="-360" max="360" value="${(shape.base * 180/Math.PI).toFixed(1)}" style="width:80px"></label></div>
            <div><label>Длина лучей: <input id="prop-len" type="number" min="20" max="400" value="${shape.len}" style="width:80px"></label></div>
          `;
          specific.querySelector('#prop-angle').addEventListener('change', (e) => {
            const v = Number(e.target.value);
            if (!Number.isNaN(v)) shape.angle = Math.max(1, Math.min(359, v)) * Math.PI / 180;
          });
          specific.querySelector('#prop-base').addEventListener('change', (e) => {
            const v = Number(e.target.value);
            if (!Number.isNaN(v)) shape.base = v * Math.PI / 180;
          });
          specific.querySelector('#prop-len').addEventListener('change', (e) => {
            const v = Number(e.target.value);
            if (!Number.isNaN(v)) shape.len = Math.max(20, Math.min(400, v));
          });
        } else {
          specific.innerHTML = '<div style="color:#666">Нет дополнительных свойств</div>';
        }
      }

      btnScaleDec.addEventListener('click', () => { if (!activeShape) return; activeShape.scale = Math.max(0.1, activeShape.scale - 0.05); updatePropsPanel(activeShape); });
      btnScaleInc.addEventListener('click', () => { if (!activeShape) return; activeShape.scale = (activeShape.scale || 1) + 0.05; updatePropsPanel(activeShape); });
      btnRotDec.addEventListener('click', () => { if (!activeShape) return; activeShape.rotation = (activeShape.rotation || 0) - Math.PI / 36; updatePropsPanel(activeShape); });
      btnRotInc.addEventListener('click', () => { if (!activeShape) return; activeShape.rotation = (activeShape.rotation || 0) + Math.PI / 36; updatePropsPanel(activeShape); });
      btnReset.addEventListener('click', () => { if (!activeShape) return; activeShape.scale = 1; activeShape.rotation = 0; updatePropsPanel(activeShape); });
    ...
    canvasEl.addEventListener('touchstart', (e) => {
      if (e.touches.length === 2 && activeShape) {
        ...
        startScale = activeShape.scale;
        startRot = activeShape.rotation;
      }
    });
    ...
    canvasEl.addEventListener('touchend', () => { touchMode = null; });
